...
class SqlTestCase extends TestCase {
    ...
    {   
        ...
        $pdo->query("CREATE UNIQUE INDEX users_username ON users (username)");

        $pdo->query("INSERT INTO users (username, password) VALUES
            ('user_a', '" . password_hash('password_a',PASSWORD_DEFAULT) . "'),
            ('user_b', '" . password_hash('password_b',PASSWORD_DEFAULT) . "'),
            ('user_c', '" . password_hash('password_c',PASSWORD_DEFAULT) . "')
        ");
